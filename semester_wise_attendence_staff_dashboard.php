...
<link rel="stylesheet" href="css/semester_wise_attendence_staff_dashboard.css">
...
<form action="semester_wise_attendence_staff_dashboard_data.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row"style="text-align:center">
                        <div class="col d-flex flex-column flex-sm-row  mb-12" >
                            <div class="col-sm-6 col-md-9 col-lg-9 col-xl-9">
                                <h2 style=" color: brown;font-weight: bolder; font-size: 25px;"><i>Semester Wise Attendance</i></h2>
                            </div>
                        </div>
                      </div>
                    </div>
                                                ...
                                                <div class="row">
                                                   <div class="col-md-12">
                                                        ...
                                                        <table class="table" border="2" style="font-size: 15px; text-align: center; background-color:lightcyan;">
                                                         <tr><td><b>S.No.</b></td><td><b>Student Name</b></td><td><b>Student Roll Number</b></td><td><b>Student Email ID</b></td><td><b>Total Present <br> in this Semster</b></td><td><b>Total Absent <br> in this Semster</b></td><td><b>Overall Attendance<br> in Percentage</b></td></tr>
                                                        <?php
														 $i=0;
														 $sum=0;
														 $k=0;
														while($row2= $rs2->fetch_array())
                                                        {
															//echo $row2[0]."<br>";
															$i++;
															$s22= "select * from take_attendance where (Semester='$sem' and Branch='$branch' and Section='$sec' and Faculty_Subject='$fs' and S_email='$row2[0]')";
                                                            $rs22=  $con->query($s22);
															$row22= $rs22->fetch_array();
															//echo $row22[0];
															
															$s3= "select COUNT(Status) from take_attendance where (S_Email='$row2[0]' and Status='present' and Semester='$sem' and Branch='$branch' and Section='$sec' and Faculty_Subject='$fs')";
                                                            $rs3=  $con->query($s3); 
	                                                        $row3= $rs3->fetch_array();
															//echo $row3[0];
															
															$s4= "select COUNT(Status) from take_attendance where (S_Email='$row2[0]' and Status='absent' and Semester='$sem' and Branch='$branch' and Section='$sec' and Faculty_Subject='$fs')";
                                                            $rs4=  $con->query($s4); 
													    ...
													    ?>
                                                          <tr><td><?php echo $i; ?></td><td><?php echo $row22[5];?></td><td><?php echo $row22[6];?></td><td><?php echo $row22[7];?></td><td><?php echo $row3[0];?></td><td><?php echo $row4[0];?></td><td><?php echo $P;?> %</td></tr>
														<?php
                                                        }
														?>
														</table>
                                                         </div><a href="download_semester_wise_attendence_staff_dashboard.php"><h2>Click here to Download Attendance</h2></a>
                                                   </div>
